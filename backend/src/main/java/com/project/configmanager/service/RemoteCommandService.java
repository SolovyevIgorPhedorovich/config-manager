...
import com.project.configmanager.config.event.TaskCompletedEvent;
import com.project.configmanager.model.command.CommandExecutionRequest;
import com.project.configmanager.model.command.CommandGroupStatus;
import com.project.configmanager.model.command.CommandTaskResult;
import com.project.configmanager.model.command.ConnectionProtocol;
import com.project.configmanager.model.command.DeviceCommandTarget;
...
public class RemoteCommandService {
    ...
    private void runDeviceTask(String command, DeviceCommandTarget target, CommandTaskResult queued, Duration timeout) {
        ...
        try {
            Map<String, String> adapterConfig = buildAdapterConfig(target, timeout);
            log.debug("Adapter config: {}", adapterConfig);
            
            adapter = adapterFactory.createAdapter(target.resolvedProtocol(), adapterConfig);
            
            if (adapter == null) {
                throw new IllegalStateException("Adapter is null for protocol: " + target.resolvedProtocol());
            }
            
            Map<String, Object> connectResult = adapter.connect(adapterConfig).get(timeout.toSeconds(), TimeUnit.SECONDS);
